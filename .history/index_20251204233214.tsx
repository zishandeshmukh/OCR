...
const Dashboard = ({ user, onLogout }: { user: UserAccount, onLogout: () => void }) => {
  ...
  // --- OPTIMIZED GEMINI CALL (Fast + Accurate) ---
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string): Promise<Voter[]> => {
     if (!apiKey) throw new Error("API Key missing");
     const ai = new GoogleGenAI({ apiKey: apiKey });
     
     // OPTIMIZED PROMPT: Concise for speed, structured for accuracy
     const prompt = `VOTER LIST EXTRACTION - Output ONLY pipe-separated values.
Format: Serial|ID|Name|Relative|HouseNo|Age|Gender|Status

RULES:
1. Extract ALL voters (~25-35 per page). Do NOT skip any.
2. Keep EXACT Marathi text. NO translation.
3. Gender: M (Male/पुरुष) or F (Female/महिला)
4. Status: A (Active) or D (Deleted/strikethrough)
5. Empty fields = leave empty
6. ID format: 3 letters + 7 digits (e.g., WUB1234567)

START OUTPUT:`;

      const parts: any[] = [{ inlineData: { mimeType: 'image/jpeg', data: base64Data }}];
      if (extractedTextHint && extractedTextHint.length > 50) {
          parts.push({ text: `REFERENCE TEXT: ${extractedTextHint.slice(0, 300)}` });
      }
      parts.push({ text: prompt });

      // SINGLE ATTEMPT with longer timeout (paid tier has better reliability)
      const maxRetries = 2;
      let delay = 1000;
        ...
        try {
            const response = await ai.models.generateContent({
                ...
                config: {
                    ...
                    maxOutputTokens: 4096, // Enough for ~35 voters
                }
            });
            ...
            return lines.map(line => {
                ...
                // Validate voter ID format (3 letters + digits)
                const id = cols[1] || '';
                const isValidId = /^[A-Z]{2,3}\d{5,10}$/i.test(id.replace(/\s/g, ''));
                ...
                return {
                    ...
                    gender: cols[6]?.toUpperCase() === 'M' || cols[6]?.includes('पुरुष') ? 'Male' : 
                            cols[6]?.toUpperCase() === 'F' || cols[6]?.includes('महिला') ? 'Female' : 'Unknown',
                    status: cols[7]?.toUpperCase() === 'D' ? 'DELETED' : 'Active',
                    confidenceScore: isValidId ? 95 : 75 // Lower score if ID seems invalid
                };
