    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 border border-slate-100 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-2">
                        {users.filter(u => u.role === 'admin').map((user, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl">
                ...
                {/* Add New Admin Form */}
                <form onSubmit={handleAddUser} className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                    <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Add New Admin (Password Protected)</h3>
