...
const DESKTOP_CONFIG = {
  ...
  // ============ ULTRA-SPEED SETTINGS FOR 20 DEVICES ============
  // Rate limiting for 20 computers sharing one key
  // Paid tier: 2000 RPM total / 20 PCs = 100 RPM each (safe limit)
  CONCURRENCY: 15, // Reduced for 20 devices to avoid rate limits
  
  // Image processing settings for MAXIMUM ACCURACY
  IMAGE_SCALE: 2.0, // Higher = better OCR accuracy
  JPEG_QUALITY: 0.85, // Higher quality for better text recognition
  
  // Pre-render settings
  RENDER_CONCURRENCY: 6, // Parallel page rendering (reduced for stability)
  
  // AI Model settings
  USE_FLASH_MODEL: true, // gemini-2.5-flash (fastest)
  ENABLE_HYBRID_OCR: true, // Use Tesseract + Gemini for verification
  
  // Batch processing
  BATCH_SIZE: 4, // Process multiple pages in single API call
...
CLASSIFICATION:`;
  ...
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string, pageNum?: number, sourceFile?: string): Promise<Voter[]> => {
     ...
     // COMPREHENSIVE PROMPT - Handles all page types including single voter
     const prompt = `Analyze this Marathi electoral roll page and extract voter data.

=== FIRST: CHECK PAGE TYPE ===
If this is a COVER PAGE (title, instructions, summary, no voter table):
Output ONLY: PAGETYPE|COVER|No voter data on this page

If this is an EMPTY PAGE:
Output ONLY: PAGETYPE|EMPTY|Blank page

=== IF VOTER DATA EXISTS: Extract all data ===

SECTION 1: PAGE HEADER (output ONCE)
...
SECTION 2: VOTER TABLE (output EACH voter, even if only 1)
VOTER|SrNo|EPIC|FullName_M|FullName_E|RelationType|RelationName_M|RelationName_E|HouseNo|Age|Sex|Status|SerialNoInPart

Per voter:
- SrNo: Serial number on page (1, 2, 3...)
- EPIC: Voter ID EXACTLY as shown (e.g., WUB1234567, YLC8904561)
- FullName_M: Full name in EXACT Marathi (do not modify spelling)
- FullName_E: Phonetic English transliteration (झिशान → Zishan)
...
- HouseNo: House/Building/Room number
- Age: Voter age (number only)
- Sex: M (पुरुष/Male) or F (महिला/Female)
- Status: ALIVE (normal) or DELETED (crossed/strikethrough)
- SerialNoInPart: Part serial if shown

CRITICAL RULES:
1. Extract EVERY voter - even if only 1 on page
2. Keep EXACT Marathi spelling - zero modifications
3. EPIC format: 2-3 letters + 7-10 digits (e.g., WUB1234567)
4. Convert ALL Devanagari numbers: ०=0, १=1, २=2...
5. Do NOT skip voters with missing fields - use empty string
6. Photo boxes indicate voter entries - count them

${extractedTextHint ? `OCR TEXT HINT: ${extractedTextHint.slice(0, 400)}` : ''}
...
OUTPUT:`;
      ...
      try {
        ...
        // Check if this is a cover/empty page
        for (const line of lines) {
          if (line.toUpperCase().includes('PAGETYPE|COVER') || 
              line.toUpperCase().includes('PAGETYPE|EMPTY')) {
            console.log(`📄 Page ${pageNum}: Skipped (${line.includes('COVER') ? 'Cover' : 'Empty'} page)`);
            return []; // Return empty array for non-data pages
          }
        }
...
OUTPUT:` });
  ...
  const handlePdfProcessing = async (file: File) => {
    ...
    try {
        ...
        // ============ ULTRA-SPEED SETTINGS FROM CONFIG ============
        const CONCURRENCY = DESKTOP_CONFIG.CONCURRENCY;        // 25 parallel API calls
        const IMAGE_SCALE = DESKTOP_CONFIG.IMAGE_SCALE;        // 2.0 for high accuracy
        const JPEG_QUALITY = DESKTOP_CONFIG.JPEG_QUALITY;      // 0.85 for clarity
        const RENDER_CONCURRENCY = DESKTOP_CONFIG.RENDER_CONCURRENCY; // 8 parallel renders
        const BATCH_SIZE = DESKTOP_CONFIG.BATCH_SIZE;          // 4 pages per API call
        const USE_BATCH_MODE = BATCH_SIZE > 1 && numPages >= 8; // Enable batch if enough pages
        // ==========================================================
        
        const startTime = Date.now();
        console.log(`🚀 ULTRA-SPEED MODE: ${numPages} pages | Concurrency: ${CONCURRENCY} | Scale: ${IMAGE_SCALE}x | Batch: ${USE_BATCH_MODE ? BATCH_SIZE : 'OFF'}`);
        ...
        const processWithGemini = async (pageData: { pageNum: number, base64: string, textHint: string }) => {
            ...
            try {
                ...
                // Show if pages are being skipped
                const skipInfo = skippedPages > 0 ? ` (${skippedPages} cover pages skipped)` : '';
                setProcessDetail(`Page ${completedCount}/${numPages} • ${totalVotersExtracted} records${skipInfo}`);
        ...
        // Show summary with skipped pages info
        const skipSummary = skippedPages > 0 ? ` | ${skippedPages} cover pages skipped` : '';
        const singleSummary = singleVoterPages > 0 ? ` | ${singleVoterPages} single-voter pages` : '';
        setProcessDetail(`✅ ${currentSessionVoters.length} records from ${dataPages} pages in ${totalTime}s${skipSummary}`);
        
        console.log(`
🚀 ULTRA-SPEED EXTRACTION COMPLETE:
   ────────────────────────────────
   📄 Total Pages: ${numPages}
   📊 Data Pages: ${dataPages}
   🚫 Cover/Empty Pages Skipped: ${skippedPages}
  ...
  const handleBulkTranslateMissing = async () => {
      ...
      // Confirm with user
      if(!confirm(`This will translate ${allCandidates.length} names using AI. This may take a few minutes. Continue?`)) {
          return;
      }
      
      addToast(`Starting translation of ${allCandidates.length} names...`, "info");
...
Output (JSON array only):`;
              ...
              try {
                  ...
                  addToast(`Translated ${completed}/${allCandidates.length} names...`, "info");
          ...
          updateVotersWithHistory(newVoters);
          addToast(`✅ Successfully translated ${transMap.size} names!`, "success");

      } catch (e) {
          console.error("Bulk Translate Error", e);
          addToast("Translation failed. Check API key and try again.", "error");
      }
  };
