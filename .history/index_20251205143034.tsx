...
const Dashboard = ({ user, onLogout }: { user: UserAccount, onLogout: () => void }) => {
  ...
  // --- ULTRA-OPTIMIZED GEMINI CALL (Maximum Speed + Accuracy) ---
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string): Promise<Voter[]> => {
     if (!apiKey) throw new Error("API Key missing");
     const ai = new GoogleGenAI({ apiKey: apiKey });
     
     // HIGHLY OPTIMIZED PROMPT - Structured for 99%+ accuracy
     const prompt = `CRITICAL: Extract EVERY voter from this election roll image.

OUTPUT FORMAT (pipe-separated, one voter per line):
Serial|VoterID|FullName|RelativeName|HouseNo|Age|Gender|Status

EXTRACTION RULES:
1. Serial: Row number (1, 2, 3...)
2. VoterID: EXACTLY as shown (format: ABC1234567)
3. FullName: Complete Marathi name - EXACT spelling
4. RelativeName: Father/Husband name in Marathi
5. HouseNo: House/Ward number
6. Age: Number only
7. Gender: M or F
8. Status: A (active) or D (deleted/crossed)

ACCURACY REQUIREMENTS:
- Extract ALL voters (typically 25-35 per page)
- Preserve EXACT Marathi characters
- VoterID must match pattern: 2-3 letters + 7-10 digits
- Do NOT skip ANY entry
- Do NOT translate names

${extractedTextHint ? `OCR REFERENCE (use to verify): ${extractedTextHint.slice(0, 600)}` : ''}
...
OUTPUT:`;
      ...
      // Single optimized request with proper config
      try {
        const response = await ai.models.generateContent({
          model: 'gemini-2.5-flash',
          contents: { parts },
          config: {
            ...
            maxOutputTokens: 8192, // Enough for 50+ voters
            topP: 0.8,
            topK: 20,
          }
        });
        ...
        // Advanced parsing with multiple validation layers
        const lines = text.split('\n').filter(line => {
          const trimmed = line.trim();
          // Must have pipes, reasonable length, not a header
          return trimmed.includes('|') && 
                 trimmed.length > 20 && 
                 !trimmed.toLowerCase().includes('serial') &&
        ...
        for (const line of lines) {
          ...
          const serialNo = cols[0]?.replace(/[^\d]/g, '') || '';
          const rawId = cols[1] || '';
          const id = rawId.replace(/\s/g, '').replace(/[oO]/g, '0').toUpperCase();
          const name = cols[2]?.trim() || '';
          const relativeName = cols[3]?.trim() || '';
          const houseNo = cols[4]?.trim() || '';
          const age = parseInt(cols[5]?.replace(/[^\d]/g, '')) || 0;
          const genderRaw = cols[6]?.toUpperCase() || '';
          const statusRaw = cols[7]?.toUpperCase() || '';
          
          // Validate voter ID format (strict)
          const isValidId = /^[A-Z]{2,3}\d{5,10}$/i.test(id);
          ...
          // Determine gender
          let gender = 'Unknown';
          if (genderRaw === 'M' || genderRaw.includes('पुरुष') || genderRaw.includes('MALE')) {
            gender = 'Male';
          } else if (genderRaw === 'F' || genderRaw.includes('महिला') || genderRaw.includes('FEMALE')) {
            gender = 'Female';
          }
          
          // Determine status
          const status = statusRaw === 'D' || statusRaw.includes('DELETE') ? 'DELETED' : 'Active';
          ...
          if (gender !== 'Unknown') confidence += 5;
          
          // Only add if minimum criteria met
        ...
        if (e.status === 429) {
          ...
          return callGeminiWithImage(base64Data, extractedTextHint);
        }
        return [];
      }
  };
    ...
    try {
        ...
        const processWithGemini = async (pageData: { pageNum: number, base64: string, textHint: string }) => {
            ...
            try {
                ...
                const voters = await callGeminiWithImage(pageData.base64, enhancedHint);
