...
const Dashboard = ({ user, onLogout }: { user: UserAccount, onLogout: () => void }) => {
  ...
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string, pageNum?: number, sourceFile?: string): Promise<Voter[]> => {
     ...
     // COMPREHENSIVE PROMPT - Extracts HEADER + ALL VOTERS
     const prompt = `Extract COMPLETE data from this Marathi electoral roll page.

=== SECTION 1: PAGE HEADER (output ONCE at top) ===
HEADER|GAT_and_Gan_Details|PartNo|BootName|BoothaAddress|PageNumber|ACNo

Extract these from the PAGE HEADER/TOP area:
- GAT_and_Gan_Details: Constituency info (e.g., "विधानसभा मतदारसंघ: 175 - पुणे कॅन्टोन्मेंट" or ward details)
- PartNo: Part number (e.g., "भाग क्रमांक: 45 - कॅम्प, पुणे")
- BootName: Polling station name (e.g., "जिजामाता प्राथमिक शाळा")
- BoothaAddress: Full booth address
- PageNumber: Page number (convert Marathi digits to English: ०=0, १=1, etc.)
- ACNo: Assembly Constituency number (e.g., "175")

=== SECTION 2: VOTER TABLE (output ALL rows) ===
VOTER|SrNo|EPIC|FullName_M|FullName_E|RelationType|RelationName_M|RelationName_E|HouseNo|Age|Sex|Status|SerialNoInPart

For EACH voter row:
- SrNo: Serial number on page
- EPIC: Voter ID exactly as shown (e.g., WUB1234567)
- FullName_M: Full name in EXACT Marathi
- FullName_E: Transliterate name to English (e.g., "झिशान देशमुख" → "Zishan Deshmukh")
- RelationType: "Father", "Husband", or "Mother"
- RelationName_M: Relation name in EXACT Marathi
- RelationName_E: Transliterate to English
- HouseNo: House/Building number
- Age: Age in numbers
- Sex: M or F
- Status: ALIVE (normal) or DELETED (crossed out/strikethrough)
- SerialNoInPart: Serial number within the part (if shown)

CRITICAL RULES:
1. Extract EVERY voter (typically 25-35 per page)
2. Keep EXACT Marathi spelling - no modifications
3. Transliterate names phonetically to English
4. Convert Devanagari digits (०१२३...) to English (0123...)
5. RelationType from "पिता", "पती", "आई" etc.

${extractedTextHint ? `OCR HINT: ${extractedTextHint.slice(0, 500)}` : ''}
...
OUTPUT:`;
      ...
      try {
        const response = await ai.models.generateContent({
          ...
          config: {
            temperature: 0.05,
            maxOutputTokens: 12000, // Larger for header + voters
            topP: 0.8,
            topK: 20,
          }
        });
