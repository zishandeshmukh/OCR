...
const DESKTOP_CONFIG = {
  ...
  EMBEDDED_API_KEY: 'AIzaSyCk7NC258QLFVZpUOy206ibEjZRh44nXIY', // Replace with your paid key
  
  // ============ ULTRA TURBO SETTINGS ============
  // Rate limiting for 10 computers sharing one key
  // Paid tier: 2000 RPM total / 10 PCs = 200 RPM each
  
  CONCURRENCY: 25,           // INCREASED: Max parallel API requests per computer
  RENDER_CONCURRENCY: 8,     // Parallel page rendering
  IMAGE_SCALE: 1.8,          // Higher quality for better accuracy
  JPEG_QUALITY: 0.75,        // Better quality for OCR
  
  // Advanced Processing
  USE_BATCH_API: true,       // Use Gemini batch processing
  ENABLE_TEXT_HINTS: true,   // Use PDF text layer as hints
  SMART_RETRY: true,         // Intelligent retry on failures
...
const Dashboard = ({ user, onLogout }: { user: UserAccount, onLogout: () => void }) => {
  ...
  // --- ULTRA TURBO GEMINI CALL (Maximum Speed + Accuracy) ---
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string): Promise<Voter[]> => {
     if (!apiKey) throw new Error("API Key missing");
     const ai = new GoogleGenAI({ apiKey: apiKey });
     
     // ADVANCED PROMPT: Structured JSON output for 99%+ accuracy
     const prompt = `You are an expert OCR system for Indian voter lists. Extract ALL voter records from this image.

OUTPUT FORMAT: Respond with ONLY a JSON array. No markdown, no explanation.
[{"s":"1","id":"WUB1234567","n":"नाम","r":"संबंधी","h":"12","a":"35","g":"M","st":"A"}]

FIELD MAPPING:
- s: Serial number (digits only)
- id: Voter ID (format: 3 uppercase letters + 7 digits, e.g., WUB1234567)
- n: Full name in EXACT original script (Marathi/Hindi/English as shown)
- r: Relative name in EXACT original script
- h: House/Address number
- a: Age (2 digits)
- g: Gender (M=Male/पुरुष, F=Female/महिला)
- st: Status (A=Active, D=Deleted/crossed out)

CRITICAL RULES:
1. Extract EVERY voter visible (~25-35 per page). Missing records = failure.
2. Keep text EXACTLY as printed. NO translation or transliteration.
3. Voter IDs are in format: 3 letters + 7 numbers (e.g., WUB3500444)
4. Look for table structure with columns: अनु.क्र., ओळखपत्र क्र., नाव, नातेवाईक, घर क्र., वय, लिंग
5. If text is unclear, use [?] marker
6. Crossed-out/strikethrough entries = status "D"
...
RESPOND WITH JSON ARRAY ONLY:`;
      ...
      // Add text hint for better accuracy
      if (DESKTOP_CONFIG.ENABLE_TEXT_HINTS && extractedTextHint && extractedTextHint.length > 30) {
          parts.push({ text: `[PDF TEXT LAYER - Use for spelling verification]: ${extractedTextHint.slice(0, 400)}` });
      }
      parts.push({ text: prompt });

      // Smart retry with exponential backoff
      const maxRetries = DESKTOP_CONFIG.SMART_RETRY ? 3 : 1;
      let delay = 500;
        ...
        try {
            const response = await ai.models.generateContent({
                ...
                config: {
                    ...
                    maxOutputTokens: 8192,  // Doubled for larger pages
                    topP: 0.8,              // Focused sampling
                    topK: 20,               // Limited vocabulary for structured output
                }
            });
            ...
            try {
                ...
                return parsed.map((item: any) => {
                    const id = (item.id || item.ID || '').toString().replace(/\s/g, '').toUpperCase();
                    const isValidId = /^[A-Z]{2,3}\d{5,10}$/.test(id);
                    
                    // Calculate confidence based on data quality
                    let confidence = 90;
                    if (isValidId) confidence += 5;
                    if (item.n && item.n.length > 2) confidence += 3;
                    if (item.a && parseInt(item.a) >= 18 && parseInt(item.a) <= 120) confidence += 2;
                    confidence = Math.min(100, confidence);
                    
                    return {
                        serialNo: (item.s || item.serial || '').toString().replace(/[^\d]/g, ''),
                        id: id,
                        name: item.n || item.name || '',
                        nameEn: '',
                        relativeName: item.r || item.relative || '',
                        relativeNameEn: '',
                        houseNo: (item.h || item.house || '').toString(),
                        age: parseInt(item.a || item.age) || 0,
                        gender: (item.g || item.gender || '').toUpperCase() === 'M' ? 'Male' : 
                                (item.g || item.gender || '').toUpperCase() === 'F' ? 'Female' : 
                                (item.g || '').includes('पुरुष') ? 'Male' :
                                (item.g || '').includes('महिला') ? 'Female' : 'Unknown',
                        status: (item.st || item.status || '').toUpperCase() === 'D' ? 'DELETED' : 'Active',
                        confidenceScore: confidence
                    };
                }).filter((v: Voter) => v.name && v.name.length > 0);
                
            } catch (parseError) {
                console.warn('JSON parse failed, trying fallback pipe format');
                
                // Fallback: Try pipe-separated parsing
                const lines = text.split('\n').filter(line => 
                    line.includes('|') && line.trim().length > 15
                );
                ...
                return lines.map(line => {
                    ...
                    const id = (cols[1] || '').replace(/\s/g, '');
                    return {
                        serialNo: cols[0]?.replace(/[^\d]/g, '') || '',
                        id: id,
                        name: cols[2] || '',
                        nameEn: '',
                        relativeName: cols[3] || '',
                        relativeNameEn: '',
                        houseNo: cols[4] || '',
                        age: parseInt(cols[5]?.replace(/[^\d]/g, '')) || 0,
                        gender: cols[6]?.toUpperCase() === 'M' ? 'Male' : 'Female',
                        status: cols[7]?.toUpperCase() === 'D' ? 'DELETED' : 'Active',
                        confidenceScore: 80
                    };
                }).filter(v => v !== null && v.name.length > 0) as Voter[];
            }

        } catch (e: any) {
            console.warn(`Gemini Attempt ${attempt + 1}:`, e.status || e.message);
            if (e.status === 429 || e.status === 503 || e.status === 500) {
                await new Promise(resolve => setTimeout(resolve, delay));
                delay *= 2;
            } else if (attempt === maxRetries - 1) {
                console.error("Gemini failed permanently:", e);
            }
        }
      }
      return [];
  ...
  const handlePdfProcessing = async (file: File) => {
    ...
    try {
        ...
        // ============ TURBO MODE SETTINGS ============
        // Uses config for shared API key across 10 computers
        // Paid tier: 2000 RPM / 10 PCs = 200 each, we use 15 for safety
        const CONCURRENCY = DESKTOP_CONFIG.CONCURRENCY; // From config
