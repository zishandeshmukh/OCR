...
const LoginScreen = ({ onLogin }: { onLogin: (user: UserAccount) => void }) => {
    ...
    const handleEmployeeLogin = (e: React.FormEvent) => {
        ...
        // Create employee session (no password needed)
        const employeeUser: UserAccount = { 
            username: name, 
            password: '', 
            role: 'employee' 
        };
        ...
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center items-center p-4">
                ...
                <p className="mt-8 text-xs text-slate-400">
                    v1.0.0 • Employees: Enter your name • Admin: Use password
                </p>
            </div>
        );
    }

    // Employee login (just name)
        ...
        return (
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center items-center p-4">
                <div className="mb-8 text-center">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-2">Enter your name to continue</p>
                </div>
                    ...
                    <form onSubmit={handleEmployeeLogin} className="space-y-6">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all dark:text-white text-lg"
                                placeholder="e.g., Ramesh, Priya, Amit"
                                autoFocus
                            />
                            <p className="text-xs text-slate-400 mt-2">This name will be attached to your extracted records</p>
                        </div>
...
// 2. USER MANAGEMENT MODAL (ADMIN ONLY) - Shows registered users AND active employees
const UserManagementModal = ({ onClose }: { onClose: () => void }) => {
    const [users, setUsers] = useState<UserAccount[]>(getUsers());
    const [newUsername, setNewUsername] = useState('');
    const [newPassword, setNewPassword] = useState('');
    const [error, setError] = useState('');
    const [activeEmployees, setActiveEmployees] = useState<{name: string, count: number}[]>([]);
    const [changingPasswordFor, setChangingPasswordFor] = useState<string | null>(null);
    const [newAdminPassword, setNewAdminPassword] = useState('');
    const [passwordSuccess, setPasswordSuccess] = useState('');

    // Load active employees from database (those who have extracted data)
    useEffect(() => {
            ...
            try {
                ...
                request.onsuccess = () => {
                    ...
                    voters.forEach((v: any) => {
                        if (v.extractedBy) {
                            employeeCounts[v.extractedBy] = (employeeCounts[v.extractedBy] || 0) + 1;
                        }
                    });
    ...
    const handleAddUser = (e: React.FormEvent) => {
        ...
        const newUser: UserAccount = { username: newUsername, password: newPassword, role: 'admin' };
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg p-6 border border-slate-100 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
                ...
                {/* Active Employees (from extraction data) */}
                {activeEmployees.length > 0 && (
                    <div className="mb-6">
                        ...
                        <div className="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-xl border border-emerald-200 dark:border-emerald-800">
                            <div className="space-y-2 max-h-40 overflow-y-auto">
                                {activeEmployees.map((emp, idx) => (
                                    <div key={idx} className="flex items-center justify-between p-2 bg-white dark:bg-slate-800 rounded-lg">
                        ...
                        <p className="text-xs text-slate-400 mt-2">
                            💡 Employees login with their name only (no password needed)
                        </p>
                    </div>
                )}
                ...
                <div className="mb-6">
                    ...
                    {passwordSuccess && (
                        <div className="mb-3 p-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-sm rounded-lg flex items-center gap-2">
                            ✅ {passwordSuccess}
                        </div>
                    )}
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-200 dark:border-amber-800">
                    ...
                    <ul className="text-sm text-amber-800 dark:text-amber-300 space-y-1">
                        <li>• Employees click <strong>"👤 Employee Login"</strong> on start screen</li>
                        <li>• They enter their <strong>name only</strong> (no password needed)</li>
                        <li>• Their name is attached to all records they extract</li>
                        <li>• They can only delete <strong>their own</strong> extracted data</li>
                        <li>• Active employees appear above once they extract data</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    useEffect(() => {
        ...
        if(sessionUser) {
            ...
            if (parsed.role === 'admin') {
                ...
                // Clear employee session - they need to login with their name each time
