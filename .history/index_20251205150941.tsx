...
const ExportModal = ({ onClose, onExport, onClear, isAdmin = false }: { onClose: () => void, onExport: (config: ExportConfig) => void, onClear: () => void, isAdmin?: boolean }) => {
        ...
        <div className="mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between gap-3">
          {isAdmin ? (
            ...
            <div className="px-4 py-2 text-xs text-slate-400 flex items-center gap-2">
              <Lock className="w-3 h-3" /> Admin only: Clear Database
            </div>
          )}
...
const Dashboard = ({ user, onLogout }: { user: UserAccount, onLogout: () => void }) => {
  ...
  const handleClearAllData = async () => {
      ...
      if (!permissions.canClearDatabase) {
        addToast('❌ Permission denied. Only admins can clear the database.', 'error');
        return;
      }
      await clearDB();
...
🚀 ULTRA-SPEED EXTRACTION COMPLETE:
  ...
  const handleDeleteAllFiltered = () => {
    // Check permission
    if (!userPermissions.canDeleteRecords) {
      addToast('❌ Permission denied. Only admins can delete records.', 'error');
      return;
    }
  ...
  const handleBulkTranslateMissing = async () => {
      ...
      try {
          ...
          if (useFreeApi) {
              ...
              const uniqueNames: string[] = [...new Set(allCandidates.map(c => c.name).filter((n): n is string => !!n))];
...
Output (JSON array only):`;
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 transition-colors duration-300">
        ...
        {selectedIds.size > 0 && (
          <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-6 z-40 animate-in slide-in-from-bottom-5 fade-in duration-300">
            ...
            <div className="flex items-center gap-2">
              {/* Delete button - shows for all but only works for admin */}
              <button onClick={handleBulkDelete} className={`flex items-center gap-2 px-3 py-1.5 hover:bg-slate-800 dark:hover:bg-slate-100 rounded-lg transition-colors text-sm font-medium ${userPermissions.canDeleteRecords ? 'text-rose-400 hover:text-rose-300 dark:text-rose-600 dark:hover:text-rose-700' : 'text-slate-500 dark:text-slate-400 opacity-50'}`} title={userPermissions.canDeleteRecords ? 'Delete selected' : 'Admin only'}>
                <Trash className="w-4 h-4" />
                Delete {!userPermissions.canDeleteRecords && '🔒'}
              </button>
      ...
      {showExportModal && <ExportModal onClose={() => setShowExportModal(false)} onExport={executeExport} onClear={handleClearAllData} isAdmin={user.role === 'admin'} />}
