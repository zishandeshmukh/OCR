...
OUTPUT:` });
  ...
  const handlePdfProcessing = async (file: File) => {
    ...
    try {
        ...
        setProcessDetail(`✅ ${currentSessionVoters.length} records in ${totalTime}s (${pagesPerMin} pages/min)`);
        
        console.log(`
🚀 ULTRA-SPEED STATS:
   Pages: ${numPages}
   Voters: ${currentSessionVoters.length}
   Duplicates removed: ${duplicates}
   Time: ${totalTime}s
   Speed: ${pagesPerMin} pages/min | ${votersPerSec} voters/sec
   Accuracy: ${avgConf}%
   Mode: Concurrency=${CONCURRENCY}, Scale=${IMAGE_SCALE}x, Hybrid=${DESKTOP_CONFIG.ENABLE_HYBRID_OCR}
        `);
