                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                        {users.map((user, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl">
                                ...
                                {user.role !== 'admin' && (
                                    <button onClick={() => handleDeleteUser(user.username)} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors" aria-label={`Delete user ${user.username}`}>
...
CLASSIFICATION:`;
  ...
  const callGeminiWithImage = async (base64Data: string, extractedTextHint?: string, pageNum?: number, sourceFile?: string): Promise<Voter[]> => {
     ...
     // COMPREHENSIVE PROMPT - ALWAYS try to extract voter data
     const prompt = `Analyze this Indian electoral roll page (Marathi/Hindi) and extract ALL voter data.

⚠️ IMPORTANT: ALWAYS extract voter data if you see ANY of these:
- Photo boxes (even empty/placeholder photos)
- Serial numbers (1, 2, 3... or क्रमांक)
- Voter ID numbers (like WUB1234567, YLC8904561)
- Names in Marathi/Hindi script
- Age numbers
- House numbers
- Text like "पिता", "पती", "आई", "पुरुष", "महिला"

🚫 ONLY mark as COVER/EMPTY if the page has:
- ONLY a large title/logo with NO voter entries at all
- Completely blank/white page
- Only instructions text with zero voter data

=== IF YOU SEE ANY VOTER DATA (even partial): Extract it! ===
...
7. If you see a table structure with names/IDs - it's voter data, NOT a cover page!

${extractedTextHint ? `OCR TEXT HINT: ${extractedTextHint.slice(0, 400)}` : ''}

OUTPUT (start with HEADER, then VOTER lines):`;
      ...
      try {
        ...
        // Check if ANY line has voter data pattern (EPIC ID, name, etc.)
        const hasVoterPattern = lines.some(line => {
          const upper = line.toUpperCase();
          // Look for EPIC patterns (2-3 letters + 7 digits)
          if (/[A-Z]{2,3}\d{7,10}/.test(upper)) return true;
          // Look for VOTER| prefix
          if (upper.startsWith('VOTER|')) return true;
          // Look for age patterns
          if (/\|\d{2,3}\|[MF]\|/.test(upper)) return true;
          return false;
        });
        
        // ONLY skip if EXPLICITLY marked as cover/empty AND no voter patterns found
        if (!hasVoterPattern) {
          for (const line of lines) {
            if (line.toUpperCase().includes('PAGETYPE|COVER') || 
                line.toUpperCase().includes('PAGETYPE|EMPTY') ||
                line.toUpperCase().includes('NO VOTER DATA')) {
              console.log(`📄 Page ${pageNum}: Skipped (Cover/Empty page - no voter patterns)`);
        ...
        for (const line of lines) {
          ...
          // Validation flags
          const isValidId = /^[A-Z]{3}\d{7}$/.test(id);
